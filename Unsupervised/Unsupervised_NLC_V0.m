...
NLC_matrice= NLC(adj_train);
NLC_feature=Conv2EdgeList(edges_train, NLC_matrice);
NLC_feature= cat(2,NLC_feature,edges_train(:,1:2));
...
NLC_sorted=sort_column1(NLC_feature);
label=zeros(length(NLC_sorted),1);
NLC_sorted=cat(2,NLC_sorted,label);
for i=1:length(NLC_sorted)
    if(findedge(G,NLC_sorted(i,2),NLC_sorted(i,3))>0)
        NLC_sorted(i,4)=1;
    end
end
...
predicted_topL=cat(1,ones(L_top,1), zeros(size(NLC_sorted,1)-L_top,1));
eval_par(j,:)=Evaluate(NLC_sorted(:,4),predicted_topL);
sum_eval_par = sum_eval_par + Evaluate(NLC_sorted(:,4),predicted_topL); 

% Actual labels: CN_sorted(:,4), scores=CN_sorted(:,1)
[X,Y,T,AUC] = perfcurve(NLC_sorted(:,4), NLC_sorted(:,1),'1');
