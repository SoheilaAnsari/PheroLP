 ...
 if (triangle (1,1) ~= i && triangle (1,2) ~= i && triangle (1,3) ~= i)
    ...
    elseif (lnk_cnt == 2)
        ...
        score = nSubgrB+tau(triangle(1,1),triangle(1,2))+...
            ...
            tau(triangle(1,1), i) = tau(triangle(1,1), i) + round(score/3);
            tau(triangle(1,2), i) = tau(triangle(1,2), i) + round(score/3);
            tau(triangle(1,3), i) = tau(triangle(1,3), i) + round(score/3);
            % Tau matrix should be symmetric
            tau(i, triangle(1,1)) = tau(i, triangle(1,1)) + round(score/3);
            tau(i, triangle(1,2)) = tau(i, triangle(1,2)) + round(score/3);
            tau(i, triangle(1,3)) = tau(i, triangle(1,3)) + round(score/3);
